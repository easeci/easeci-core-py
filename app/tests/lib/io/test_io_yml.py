...
import subprocess
import yaml
from app.lib.io.io_yml import yml_load, yml_get, yml_create, yml_append, yml_change, yml_delete
...
def create_test_yml_file():
    ...
    print(pwd)
    yml_test_file = pwd + '/app/tests/lib/io/test_config.yml'
    subprocess.check_call('cp ' + yml_test_file + ' ' + tmp_path, shell=True)


def create_test_malformed_yml_file():
    pwd = str(subprocess.check_output('pwd', shell=True))[2:-3]
    yml_test_file = pwd + '/app/tests/lib/io/test_malformed_config.yml'
